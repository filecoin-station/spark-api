...
export async function startRoundTracker ({ pgPool, signal }) {
  ...
  const onRoundStart = (newRoundIndex, ...args) => {
    ...
    if (blockNumber === undefined) {
      console.warn('Ethers.js event data did not include ContractEvent with the block number')
    }

    updateSparkRound(pgPool, contract, newRoundIndex, event.log.blockNumber).catch(err => {
