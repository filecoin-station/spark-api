...
// Filecoin (and FEVM) has block time of approx. 30 seconds.
// This may change when we deploy the smart contract to an IPC subnet.
export const CHAIN_BLOCK_TIME = 30_000 /* 30 seconds */
...
export async function createRoundGetter (pgClient) {
  ...
  const updateSparkRound = async (meridianRoundIndex) => {
    sparkRoundNumber = await mapCurrentMeridianRoundToSparkRound({
      ...
      meridianNextRoundLength: await contract.nextRoundLength(),
      pgClient
    })
...
export async function mapCurrentMeridianRoundToSparkRound ({
  ...
  // TODO: replace "new Date()" with the time when the NextRound event was committed on the chain
  const roundDeadline = new Date(Date.now() + meridianNextRoundLength * CHAIN_BLOCK_TIME)

  await maybeCreateSparkRound(pgClient, { sparkRoundNumber, roundDeadline })

  return sparkRoundNumber
}

export async function maybeCreateSparkRound (pgClient, { sparkRoundNumber, roundDeadline }) {
  ...
  if (rowCount) {
    ...
    console.log('Spark round deadline:', roundDeadline)
  }
}
